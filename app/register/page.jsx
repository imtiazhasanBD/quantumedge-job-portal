...
export default function AuthPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        setMessage(`✅ ${mode === "login" ? "Login" : "Account"} successful!`);
      } else {
        setMessage(
          `❌ ${
            data?.email ||
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-20 relative">
      <div className="bg-[#0e1a0e] text-white p-8 flex gap-10 rounded-2xl overflow-hidden shadow-lg max-w-5xl w-full relative z-20">
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] h-[300px] bg-custom-green p-8 rounded-full blur-3xl shadow-lg opacity-35 z-10"></div>
      </div>

      {/* Blurred backdrop circles */}
      <div class="absolute top-0 left-1/12 -translate-x-1/2 -translate-y-1/2 w-[350] h-[350px] bg-custom-green p-8 rounded-full blur-3xl shadow-lg opacity-40 z-10"></div>
    </div>
  );
}
